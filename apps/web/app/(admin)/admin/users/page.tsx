...
export default function AdminUsersPage() {
  ...
  const submit = () => {
    const clean = { ...form };
    if (!clean.organizationId) delete clean.organizationId;

    if (editing) {
      const { password, email, ...rest } = clean;
          ...
          >
            ...
            >
                ...
                {[
                  ...
                  <div key={f.k}>
                    ...
                    <input
                      value={form[f.k]}
                      onChange={(e) =>
                        setForm((p: any) => ({ ...p, [f.k]: e.target.value }))
                      }
                      style={INPUT}
                      className="sa-input"
                    />
              ...
              <div>
                ...
                <select
                  value={form.role}
                  onChange={(e) =>
                    setForm((p: any) => ({ ...p, role: e.target.value }))
                  }
                  style={INPUT}
                  className="sa-input"
                >
            ...
            <div style={{ padding: "0 22px 22px", display: "flex", gap: 10 }}>
              ...
              <button
                onClick={submit}
                disabled={
                  (!editing && (!form.email || !form.password)) || isPending
                }
                style={{
                  flex: 1,
                  padding: 12,
